...
```
#Excutive Summary:
In this regression analysis, first a fit between mpg and regressors is calculated. In this fit, the am variable (transmission kind) does not have significant contribution based on the P value obtained from anova function. Another fit is introduced which indicates relationship between transmission and mpg. The results show that automatic transmission is better for MPG.

# Analysis:
In this analysis the contribution of every regressor in the linear regression that predicts mpg is studied by evaluating their P values in nested models. We first start with a simple regression between mpg and cyl.
...
Now, we add other regressors to this model one by one and and anylize their necessity via anova function.
```{r}
...
The lmd12 coefficients show that the average of mpg increases by around seven units for automatic transmission with refrence to manual transmission with an mpg average of around 17.
...
summary(lmd13)$coef
```
The coefficients show that switching from manual to automatic transmission will increase the average of mpg.

This fact can be visualized as follows:
```{r, fig.width = 10, fig.height = 4}
require(ggplot2)
transmission <- mtcars$am
transmission[mtcars$am == 0] <- 'manual'
transmission[mtcars$am == 1] <- 'automatic'
